...
| Trial/civil-court sub-district | SHIPPED `judicial-subcircuit` (consolidated CountyDispatch: Will 12th-Circuit 5 + DuPage 18th-Circuit 7 + Lake 19th-Circuit 12, PA 102-0693; Kane's 16th-Circuit entry waits on a public source — its only subcircuit services are permission-locked proxies, see Backlog) | SHIPPED `municipal-court` (28) | NO HONEST ANALOG⁶ |
...
- Judicial subcircuits beyond Will — PA 102-0693 shapefiles exist (Cook + collar).
  Kane (16th Circuit) is the live case: its county GIS publishes subcircuits only
  behind permission-locked ArcGIS proxies (verified 2026-07), so the `kane` entry
  in `judicial-subcircuit` waits on the shapefile route rather than guessing.
...
- Collar-county boards (McHenry, Kendall) — per-county ArcGIS Hubs;
  Cook + Will + DuPage + Lake + Kane shipped (Kane 2026-07: the Lake recipe again — five dispatch entries, zero new layers/scrapers; the KaneCo_IL_* hosted family carries board member names, precinct board-district fields, and fire/park/library officer + office contact) (DuPage board 2026-07: boundary + coverage
...
| `library-district` | Library District | geography | CountyDispatch | Cook County GIS L20 (Library Tax District) + L19 (Library Fund) · Will County ArcGIS (`Library_District`) · DuPage County ArcGIS (`Library_Districts_`) · Lake County ArcGIS (`LakeCounty_TaxDistricts` L8) · Kane County ArcGIS (`KaneCo_IL_Districts_Library` L1) | Cook: agency name + a Type row distinguishing district vs municipal fund; Will: trustees in GIS attrs (sparse); DuPage: name-only; Lake: district office contact in GIS attrs; Kane: board president + office contact in GIS attrs | OR of cook/will/dupage/lake/kane county coverages |

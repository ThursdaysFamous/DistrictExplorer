...
- *Remaining fix (planned; not shipped here — it can't be validated from this review sandbox, whose own browser egress is proxy-blocked).* Fetch the district pages through a real browser instead of `requests`, reusing the Playwright + Chromium the `smoke-test` workflow already installs — a genuine browser passes a Cloudflare *managed* challenge legitimately (real JS execution, no evasion, no CAPTCHA-solving service). Concretely: add an `--engine {requests,playwright}` switch to `cpd_district_scraper.py` (default `playwright` in CI), have the Playwright path `page.goto` each district URL, wait for the challenge to settle (`networkidle` / retry on the challenge interstitial), and hand the rendered HTML to the existing `parse_district_page` unchanged; keep the `requests` path as the fast offline fallback. Land R3's "≥N non-null `commanderName`s" output guard at the same time so a rendered-but-empty page can't ship either, and pin scraper deps (R10) so the heuristic parser doesn't drift alongside. If a real browser *still* can't clear the challenge (Turnstile interactive mode), the honest fallback is to surface the weekly job as a **tracked failure/alert** rather than a silent red run, and leave the graceful-degradation card in place until CPD exposes the data another way.
...
| 27 | CPD scraper blocked by Cloudflare managed challenge (403). **Partial fix shipped:** station address/phone now join client-side from the CORS `Police_District_Stations_View` (+ latent zero-pad join-key bug fixed). **Remaining:** commander + CAPS contact are scrape-only (nowhere in CPD's ArcGIS org) — fetch via Playwright/Chromium (CI already installs it), keep `requests` as fallback — R11 | Medium | Medium | Pipeline |

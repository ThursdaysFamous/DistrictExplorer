...
| 18 | `.nojekyll` **added** (skips the Jekyll pass; future-proofs against Liquid-looking JS). Actions-based Pages deploy (app files only) **deferred** — needs a repo Settings→Pages source change + is outward-facing (owner decision) — R10 | Low-Med | **Low** | DevEx |

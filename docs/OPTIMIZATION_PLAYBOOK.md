...
- *Commander/CAPS scrape fix SHIPPED — `cpd_district_scraper.py` now fetches through a real browser.* Added an `--engine {auto,requests,playwright}` switch: `playwright` drives a headless Chromium that executes Cloudflare's managed-challenge JS and reads the resulting rendered HTML (no evasion — a genuine browser), handing identical markup to the unchanged `parse_district_page`; `requests` is the fast browserless path; `auto` (default) tries `requests` and transparently falls back to `playwright` the instant the finder page is blocked (403 / challenge interstitial), so the weekly job works whether or not the runner's IP is challenged. Both engines share a `_looks_like_challenge` detector. The `update-cpd-roster` workflow now installs Chromium (`python3 -m playwright install --with-deps chromium`). Also landed alongside: R3's output guard (`build_cpd_roster.py` refuses a roster with `< MIN_COMMANDERS = 15` non-null commander names — a page reword that nulls the headline field can't silently ship) and R10's dep pinning (`scripts/requirements.txt`, `requests==2.33.1`/`beautifulsoup4==4.15.0`/`playwright==1.61.0`; the ILGA job pins via `pip -c` constraints without pulling Playwright).
  ...
  - *Live CI run (`update-cpd-roster`, workflow_dispatch, 2026-07-09):* answered the open question — **Turnstile auto-cleared for headless Chromium on GitHub's IP.** `requests` was 403'd on the finder (the runner IP *is* challenged), `auto` fell back to Playwright, and the browser fetched the finder page **and** a real district page. The guard then correctly refused the result — because the rendered finder yielded only **1** district link (district 11), not 22: the full list is lazy-loaded / behind a JS map widget, something only the live site reveals. Follow-up fix (shipped): `get_district_pages` now (a) blocks on a `wait_for_function` readiness predicate until the finder DOM actually holds ≥15 district-URL patterns (tag-agnostic, so it catches links rendered as `<a>`, `<option>`, or data-attrs), and (b) self-heals by harvesting the all-districts nav from any district page it does find, looping until the set stops growing (bounded by the 22-district count). Re-running to confirm full discovery; the guard keeps a partial run from ever shipping in the meantime.
...
| 27 | ~~CPD scraper blocked by Cloudflare managed challenge (403)~~ — **fix shipped (R11):** (a) app joins station address/phone from the CORS `Police_District_Stations_View` (+ latent zero-pad join-key bug fixed); (b) `cpd_district_scraper.py` gained `--engine auto` → falls back to a real headless Chromium that clears the managed challenge, keeping `requests` as the fast path; + R3 commander-count guard and R10 dep pinning. Only open question: whether CPD's Turnstile auto-clears on a CI IP (falls back to a tracked-failure alert if not). | Medium | Medium | Pipeline |

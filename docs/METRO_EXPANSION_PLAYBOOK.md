...
**Master copy.** This file — `docs/METRO_EXPANSION_PLAYBOOK.md` in the Chicago repo (`DistrictExplorer-CHI`) — is the single source of truth for the playbook. Sibling forks carry a pointer stub at their repo root, never a full copy (the CHI and NYC copies drifted once already; reconciled July 2026). Lessons from a port are folded back **here**. The raw NYC research/build notes this playbook absorbed are archived at `docs/archive/METRO_EXPANSION_NYC.md`.

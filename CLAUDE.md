...
- Layers: 37 registered (political 11, safety 7, schools 9, geography 10); `registerLayer(` floor 15. Debug namespace `window.ChiExplorer`.
...
- **Officeholder rosters + curated lists** (`il-{senate,house}-members.json`, `congress-roster.json`, `cpd-district-info.json`, `ccpsa-district-councils.json`, `will-county-board-members.json` — all regenerated **weekly by CI** from scraper output; `school-board-members.json` hand-curated; `early-voting-sites.json` hand-transcribed per election). Service worker serves these **network-first** so a returning visitor never gets a stale officeholder.
